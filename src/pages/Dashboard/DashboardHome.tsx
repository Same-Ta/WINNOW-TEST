    ...
    const fetchAnalytics = async () => {
        try {
            ...
            const sortedApplicants = applications
                .filter(app => app.appliedAt && app.name)
                .sort((a, b) => {
                ...
                .map(app => ({
                    ...
                    date: app.appliedAt.toDate ? 
                        app.appliedAt.toDate().toLocaleDateString('ko-KR', { year: 'numeric', month: '2-digit', day: '2-digit' }).replace(/\. /g, '.').replace(/\.$/, '') : 
                        new Date(app.appliedAt).toLocaleDateString('ko-KR', { year: 'numeric', month: '2-digit', day: '2-digit' }).replace(/\. /g, '.').replace(/\.$/, ''),
                    status: app.status
                }));
